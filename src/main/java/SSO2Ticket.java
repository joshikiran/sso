...
/**
 * (C) Copyright 2000-2019 SAP SE Walldorf
 *
 * Author: SAP SE, Security Development
 * 
 * SAP SE DISCLAIMS ALL WARRANTIES WITH REGARD TO THIS SOFTWARE, INCLUDING ALL
 * IMPLIED WARRANTIES OF MERCHANTABILITY AND FITNESS, IN NO EVENT SHALL SAP SE
 * BE LIABLE FOR ANY SPECIAL, INDIRECT OR CONSEQUENTIAL DAMAGES OR ANY DAMAGES
 * WHATSOEVER RESULTING FROM LOSS OF USE, DATA OR PROFITS, WHETHER IN AN ACTION
 * OF CONTRACT, NEGLIGENCE OR OTHER TORTIOUS ACTION, ARISING OUT OF OR IN
 * CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE.
 * 
 * This class provides wrapper functionality for SSO2Ticket (SAP Logon/Assertion
 * Ticket) in Java.
 * 
 * @version 1.17 2018
 * 
...
public class SSO2Ticket {
	...
	static {
		if (System.getProperty("os.name").startsWith("Win")) {
			...
			SECLIBRARY = "libsapcrypto.dylib"; /* Mac */
	 ...
	 */
	public SSO2Ticket() {
		System.out.println(SECLIBRARY);
		init(SECLIBRARY);
		System.out.println("Loaded Seclibrary");
	}
	 ...
	 *         (String)client , [3] = (byte[])certificate [4] = (String)portalUser,
	 *         [5] = (String)authSchema, [6] = validity, [7] = ticket type (0:
	 *         logon, 1: assertion)
	 * 
	 */
	 ...
	 * @return Object array with: [0] = (String)user, [1] = (String)sysid, [2] =
	 *         (String)client , [3] = (byte[])certificate [4] = (String)portalUser,
	 *         [5] = (String)authSchema, [6] = validity
	 * 
	 */
	public static native synchronized Object[] evalAssertionTicket(String ticket, String pab, String pab_password,
			String my_sysid, String my_client) throws Exception;
	 ...
	 * 
	 */
	public static native synchronized String createAssertionTicket(String my_sysid, String my_client, String pab,
			String pab_password, String ext_sysid, String ext_client, String sap_user, String language,
			String portal_user, String auth_schema) throws Exception;
	...
	/**
	 * Evaluate a JSON Web Token (JWT) Use loadKey to load the verification key.
	 * 
	 * @param jwt            JWT token to be validated
	 * @param expectedUserId Name of the attribute which contains the user.
	 *                       Optional.
	 * @return Object array with: [0] = (String)JWT payload, [1] = (String)user Id
	 * 
	 * @throws Exception Text message with error.
	 */
	public static native synchronized Object[] evalJWToken(String jwt, String expectedUserId) throws Exception;

	/**
	 * Evaluate a JSON Web Token (JWT) Use key verification key.
	 * 
	 * @param jwt JWT token to be validated
	 * @param key Verification key
	 * @param alg Minimum Algorithm
	 * @return Object array with: [0] = (String)JWT payload, [1] = (String)algorithm
	 *         id
	 * 
	 * @throws Exception Text message with error.
	 */
	public static native synchronized Object[] evalJWTokenEx(String jwt, String key, int alg) throws Exception;
	 ...
	 * @throws Exception Text message with error.
	 */
	public static native synchronized Object[] evalJWHeader(String jwt) throws Exception;
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (getCommandOption(args, "-c")) {
				...
				if (ticket_user == null)
					ticket_user = "SAPUSER";
				ticket = createAssertionTicket(getProperty("SAP_OWN_SYSID"), getProperty("SAP_OWN_CLIENT"), null, null,
						getProperty("SAP_EXT_SYSID"), getProperty("SAP_EXT_CLIENT"), ticket_user, "E", "PORTALUSER",
						"basicauthentication");
				...
				if (getCommandParam(args, "-host") != null && getCommandParam(args, "-sysnr") != null) {
					createSAPShortCut(getProperty("SAP_EXT_SYSID"), getProperty("SAP_EXT_CLIENT"),
							getCommandParam(args, "-host"), getCommandParam(args, "-sysnr"), ticket_user, ticket);
				}
			}
			// evaluate the ticket
			if (getCommandOption(args, "-A")) {
				o = evalAssertionTicket(ticket, null, null, getProperty("SAP_EXT_SYSID"),
			...
			if (o[3] != null && o[3] instanceof byte[]) {
				...
				/*
				 * remark: The "certificate" object is a DER encoded X.509 certificate of the
				 * issuing system, which can be parsed/analyzed with JAVA functionality e.g.
				 * Java Cryptography Architecture API, IAIK and so on.
				 */
			} // or
			if (o[7] != null && o[7] instanceof String) {
				String sType = (String) o[7];
				tType = Integer.parseInt(sType);
			}

			// print out all parameters received from SAPSSOEXT
			PrintResults(tType, (String) o[0], (String) o[1], (String) o[2],
					parseCertificate(certificate, ISSUER_CERT_SUBJECT),
					parseCertificate(certificate, ISSUER_CERT_ISSUER), ticket, (String) o[4], (String) o[5],
					(String) o[6]);
		} catch (Exception e) {
	...
	static void PrintResults(int ticketType, String user, String sysid, String client, String subject, String issuer,
			String ticket, String prtUsr, String authS, String validity) {
		System.out.println("***********************************************");
	...
	public static void createSAPShortCut(String sysId, String client, String host, String sysnr, String user,
			String ticket) throws FileNotFoundException {
		try {
	...
	static void PrintHelp() {
		...
		System.out.println(
				"   [-host <host>] [-sysnr <system number>] (create SAP Shortcut if -c, -host, -sysnr provided)");
	...
	public static Object[] evalLogonTicket(String ticket, String crt) throws Exception {
		System.out.println("Executing the evaluation");
		System.out.println("ticket is " + ticket);
		byte[] keyfile = getBytesFromFile(crt);
		System.out.println(keyfile);
		//loadKey(keyfile, null, 0, 1);
		return evalLogonTicket(ticket, null, null);
	}
}
